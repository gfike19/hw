...
def print_menu():
    ...
    print("5 to see students whose combined score exceeds the combined average score")
...
def find_avg(dict_scores):
    avgScore = 0
    for key in dict_scores:
        avgScore = avgScore + dict_scores[key][0] + dict_scores[key][1]
    comboScore = avgScore / len(dict_scores)
    return comboScore

def match_avg(dict_scores):
    avg = find_avg(dict_scores)
    print("The average score from all tests is: ", avg)
    for stdname in dict_scores:
        combined_score = dict_scores[stdname][0] + dict_scores[stdname][1]
